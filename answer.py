...
pertanyaan_cols = [col for col in df.columns if col.startswith('Q') and col[1:].isdigit() and 1 <= int(col[1:]) <= 17]
...
if target_question == "q1":
    ...
    # Catatan: q7 dan q8 sama-sama untuk TS, tapi sesuai permintaan soal
    q, count, persen = get_max_question_for_scale('TS')
    print(f"{q}|{count}|{persen:.1f}")
